...
void Master::mark_MUS(MUS& f, bool block_unex){	
    ...
    //attempt to get a complementing UNSAT subset (MUS)
    Formula complement(dimension, false);
    for(int i = 0;  i < dimension; i++){
        if(!f.bool_mus[i])
            complement[i] = true;
    }
    BooleanSolver *msSolver = static_cast<BooleanSolver*>(satSolver);
    //remove clauses that are connected with the MUS via a flip edge
    for(auto c: f.int_mus){
        msSolver->compute_flip_edges(c);
    ...
    if(!is_valid(complement, true, false)){
        cout << "disjoint pair" << endl;
        print_formula_int(f.bool_mus);
        print_formula_int(complement);
        exit(0);
    }
